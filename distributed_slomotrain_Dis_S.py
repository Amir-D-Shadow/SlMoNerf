...
time_pose_net = LearnTime(in_feat=9,mid_feat=32,out_feat=27).to(device)
nerf_model = Nerf(tpos_in_dims=90, dir_in_dims=27, D=256).to(device)
...
for epoch_i in tqdm(range(global_step,N_EPOCH), desc='Training'):
    ...
    print(f"epoch {epoch_i}: Training PSNR {train_psnr}, estimated fx {fxfy[0]} fy {fxfy[1]}")
    ...
    if args.local_rank == 0:
        if (epoch_i+1) % SAVE_WEIGHTS_INTERVAL == 0:
        ...
        with torch.no_grad():  
            ...
            if (epoch_i+1) % EVAL_INTERVAL == 0:
                ...
                fxfy = de_parallel(focal_net)()
                rendered_img, rendered_depth = render_novel_view(TSteps//2,eval_c2w, H, W, fxfy, ray_params, nerf_model,time_pose_net)
                imageio.imwrite(os.path.join(f"{os.getcwd()}/nvs_midImg/{scene_name}", scene_name + f"_img{epoch_i+1}_SlMo_Dis.png"),(rendered_img*255).cpu().numpy().astype(np.uint8))
                imageio.imwrite(os.path.join(f"{os.getcwd()}/nvs_midImg/{scene_name}", scene_name + f"_depth{epoch_i+1}_SlMo_Dis.png"),(rendered_depth*200).cpu().numpy().astype(np.uint8))
