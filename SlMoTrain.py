    ...
    for t in timeSteps:
        ...
        for intermediateInpidx in range(ray_params.num_inp_frame):
            ...
            for i in ids:
                ...
                row_list = [(rs_i,rs_i+172) for rs_i in range(H-172+1)]
                ...
                with torch.no_grad():
                    ...
                    ssim_loss_epoch.append(SSIM_loss(ssim_syn,ssim_tgt).clone().detach())
...
nerf_model = Nerf(tpos_in_dims=126, dir_in_dims=27, D=256)
if load_model:

    nerf_model.load_state_dict(torch.load(f"{model_weight_dir}/{scene_name}_nerf.pt"))
