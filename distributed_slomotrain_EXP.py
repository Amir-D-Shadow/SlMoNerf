...
load_model = False
N_EPOCH = 50  # set to 1000 to get slightly better results. we use 10K epoch in our paper.
EVAL_INTERVAL = 2  # render an image to visualise for every this interval.
...
def train_one_epoch(images_data, H, W, ray_params, opt_nerf, opt_focal,opt_pose,opt_time, nerf_model, focal_net, pose_param_net,time_pose_net,flowComp,ArbTimeFlowIntrp,flowBackWarp):
    ...
    for t in time_list:
        ...
        for intermediateInpidx in range(ray_params.num_inp_frame):
            ...
            #set up random row col for patches     
            row_size,col_size = 32,64 
...
for epoch_i in tqdm(range(global_step,N_EPOCH), desc='Training'):
    ...
    print(f"epoch {epoch_i}: Training PSNR {train_psnr}, SSIM: {Tr_loss[1]}, Total Loss: {Tr_loss[2]}, estimated fx {fxfy[0]} fy {fxfy[1]}")
