...
with torch.no_grad():
    ...
    for idx in range(4):
        ...
        #fixed time
        #i = idx*(num_steps//4)+1
        #novel_img, novel_depth = render_novel_view(40/num_steps,spiral_c2ws[i], novel_H, novel_W, novel_fxfy,ray_params, nerf_model,time_pose_net)

        #fixed view
        #i = idx*(num_steps//4)
        i = idx + 5
        eval_c2w = torch.eye(4, dtype=torch.float32) 
        novel_img, novel_depth = render_novel_view(t[i],eval_c2w, novel_H, novel_W, novel_fxfy,ray_params, nerf_model,time_pose_net)

        #novel_img_list.append(novel_img)
        #novel_depth_list.append(novel_depth)
        imageio.imwrite(os.path.join(f"{os.getcwd()}/test", scene_name + f"_img{idx+1}_SlMo.png"),(novel_img*255).cpu().numpy().astype(np.uint8))
        imageio.imwrite(os.path.join(f"{os.getcwd()}/test", scene_name + f"_depth{idx+1}_SlMo.png"),(novel_depth*200).cpu().numpy().astype(np.uint8))
